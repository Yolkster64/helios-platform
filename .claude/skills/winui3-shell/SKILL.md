...
- `references/winui3-and-legacy-wpf.md` — the shell as it ships (project shape, DI in `App`, the REST-only rule, view-model threading, immutable rows, theme brushes that survive contrast changes, windowing, the `/v1/metrics` seam a dashboard may draw), then the legacy WPF inventory and the reading/migration rules — WPF is read for intent and migrated, never written, compiled, or bridged (ADR-0010; the HC-002 baseline) — plus which `config/aihub.json` chain each kind of shell work routes to. System view: `docs/architecture/AIHUB_LANGUAGE_ROLES.md`.
