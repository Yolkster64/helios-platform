...
| `connect-devices.ps1` | **Both device codes in one sitting, then the chain.** Verify-first (`gh auth status --active`, `az account get-access-token --output none`); the lanes still logged out start together as two children with stdin closed (gh then prints its code instead of waiting for Enter) and `GH_TOKEN`/`GITHUB_TOKEN` cleared for gh; one table with both codes and URLs; both polled until they exit or `-TimeoutMinutes` passes, each lane `ready` (re-verified) / `expired` (`-Retry` re-issues once) / `refused` (AADSTS, declined) / `failed`. Both ready → the chain: `GITHUB_MODELS_TOKEN` export (dot-sourced only: `. scripts/bootstrap/connect-devices.ps1`), `connect-github-app.ps1 -DispatchGovernance` (`-SkipGitHubApp`), `connect-admin.ps1 -SkipGitHub` (`-SkipOpsIdentity`), `auto-login.ps1` (dot-sourced when this script is), `auth-doctor.ps1 -Json`, `first-run.sh --verify-only`; `-SkipChain` stops after the logins. `-VerifyOnly` probes and prints the plan, launches nothing; `-Json` one object; `-Tenant` / `-Repository`. Exit 0 / 1 (a chained script failed, replay list) / 2 (a login needs the owner or a precondition is missing). `connect-devices.sh` is the bash twin (`--kebab` flags; `source` it to export). Offline suite: `scripts/verify/tests/test_connect_devices.ps1`. |
| `connect-github-app.ps1` | **The GitHub App that replaces the admin PAT** (owner's machine, Cloud Shell or a Codespace — never an agent container: the anonymous `/rate_limit` control runs first and an injecting transport stops the script with exit 2). Verify-first reads the repository variables `HELIOS_APP_CLIENT_ID` / `HELIOS_APP_ID` / `HELIOS_APP_SLUG`, the presence of the `HELIOS_APP_PRIVATE_KEY` secret (name only) and the installation via `gh api /user/installations`. Not registered → the manifest flow: a self-submitting page opens `https://github.com/settings/apps/new?state=<nonce>` with the manifest (administration/contents/issues/pull_requests/pages/actions/workflows write, metadata read, no webhook, not public); one click **Create GitHub App**; GitHub sends the one-hour single-use code to a one-request `localhost` listener (`-CallbackPort 0` picks a port, `-1` = paste the code from the address bar, `-FromCode <ENV NAME>` = already obtained), the state nonce is verified (CSRF), `POST /app-manifests/{code}/conversions` answers with id/slug/client_id/pem (client_secret and webhook_secret are discarded); the three variables are stored with `gh variable set`, the PEM with `gh secret set` over stdin. Then one click **Install** on the pre-targeted install page; the script polls with an in-process app JWT (RS256, 9-minute exp) until the installation exists and carries `administration=write`. `-DispatchGovernance` then runs `gh workflow run governance-apply.yml -f apply=true -f scope=all`, which now mints an installation token (`actions/create-github-app-token@v3`) ahead of `HELIOS_ADMIN_TOKEN`. `-VerifyOnly` / `-Json` / `-Repository` / `-AppName` / `-TimeoutMinutes` / `-SkipInstallWait`. Exit 0 / 1 / 2. Offline suite: `scripts/verify/tests/test_connect_github_app.ps1`. |
