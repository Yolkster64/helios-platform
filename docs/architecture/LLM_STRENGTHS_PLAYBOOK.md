...
| `hermesFleet.maxConcurrentLanes` | Parallel lanes per board, independent of pool size | code 6 / infra 4 / review 9 / native 3 | `config/fleet/fleet-topology.json` | Bound it to the pool's provider quota — review fans out widest by design |
...
| Absorption benchmark parallelism | Concurrent absorb-pr benchmarks on the fleet | uncapped (`-Max` 0) | `scripts/fleet/seed-absorption-tasks.ps1 -Max <n>` (board `xcore-infra`, lane `infrastructure`) | Each benchmark is a full build+test gate — cap `-Max` below the infra pool's `maxConcurrentLanes` (4) or it starves other infra work; re-seeding is idempotent (`absorb-pr-<N>` ids) |
| Hub fan-out | Providers hit in parallel per request | `compare`/`tandem` provider list; per-provider circuit breakers | `config/aihub.json` chains | Backpressure is by design: a 429 blocks the lane's task on the board while the breaker routes other traffic to fallbacks |
