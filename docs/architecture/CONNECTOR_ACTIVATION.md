...
|---|---|---|
| Linear | Curated issue mirror into the single HELIOS project under team `JOH`; validate unarchived team/project and membership before mutation; live GitHub title/labels/state; team-scoped mirror lookup including archived records; ambiguity stops; GraphQL errors and unsuccessful mutations fail | Authorized Actions credential; one existing issue updates once without a duplicate |
...
`config/connectors.json` contains policy, navigation IDs and credential variable names only.
...
The `channels`/`channelIds` mappings and `botTokenEnv` are future service metadata; setting them
...
its use by this notifier has not been verified. The existing operator front door is
the [HELIOS Slack canvas](https://helios-xk97943.slack.com/docs/T0B8Z1H0MV1/F0BGVRND8GK).
Its workspace is `T0B8Z1H0MV1`, and the control channel is `C0BHWDBHG1W`.
Editing this canvas proves a document update; it does not prove webhook delivery.

The [single HELIOS project](https://linear.app/641974/project/helios-4f592efea071)
has ID `3a4dee89-2c83-4bbd-a067-4e6cfbcb308a`. The project overview follows
**Connect → Unify → Automate → Validate**. New mirrors must return this project
ID and a real issue identifier before a GitHub receipt can be written. Existing
mirrors outside this project stop for reconciliation, rather than creating a twin.
...
with `queue: max` (up to 100 pending runs; further runs are canceled).
...
3. Confirm team John (`JOH`), the single HELIOS project, one-way issue sync, and
   the intended Slack channel. Native issue mirroring back into GitHub must be off.
...
Source recovery: the original reliability delta from
[PR #136](https://github.com/Yolkster64/helios-platform/pull/136), commit
`6723fb559e929fe3fe99051ff1baf918c43d31b3`, is reconciled with Claude's published
`enabled` switches and mirror-loop guard. The project binding and validated
creation receipts extend that delta. This does not close or supersede the source PR.
...
[GitHub concurrency queues](https://docs.github.com/actions/writing-workflows/choosing-what-your-workflow-does/control-the-concurrency-of-workflows-and-jobs).
